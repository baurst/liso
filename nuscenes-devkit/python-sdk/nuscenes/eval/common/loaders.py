...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False, class_mapping: Dict = {}) -> EvalBoxes:
    """
...
def filter_eval_boxes(nusc: NuScenes,
    ...
    total, dist_filter, point_filter, bike_rack_filter, class_mapping_filter = 0, 0, 0, 0, 0
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        if class_mapping:
            ...
            for box in eval_boxes.boxes[sample_token]:
                simple_name = box.__getattribute__(class_field)
                detection_name = class_mapping[simple_name]
                if detection_name is None:
                    continue
                else:
                    mapped_boxes.append(box)
            eval_boxes.boxes[sample_token] = mapped_boxes

        class_mapping_filter += len(eval_boxes.boxes[sample_token])
    ...
    if verbose:
        ...
        print("=> After class based filtering: %d" % class_mapping_filter)

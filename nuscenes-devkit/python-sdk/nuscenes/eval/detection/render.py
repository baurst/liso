...
# to comply with Apache License: Modified by baurst 2023 to work headless, black formatting
# fmt: off
...
def summary_plot(md_list: DetectionMetricDataList,
                 ...
                 min_recall: float,
                 dist_th_tp: float,
                 target_class_names=DETECTION_NAMES,
                 savepath: str = None) -> None:
    """
    ...
    if len(axes.shape) == 1:
        axes = axes[None,...]
    for ind, detection_name in enumerate(target_class_names):
...
def detailed_results_table_tex(metrics_path: str, output_path: str, target_class_names=DETECTION_NAMES) -> None:
    """

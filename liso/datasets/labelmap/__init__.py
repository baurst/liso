...
def read_yaml_pythonic(path_to_file: str):
...
def get_label_map_from_file(
    ...
    labelmap_file = (
        Path(getsourcefile(lambda: 0)).parent.parent.parent
        / "config"
        / "label_mappings.yml"
    )
    label_map_cfgs = read_yaml_pythonic(labelmap_file)
    label_map = LabelMap(

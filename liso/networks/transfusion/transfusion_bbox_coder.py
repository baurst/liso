...
class TransFusionBBoxCoder(BaseBBoxCoder):
    ...
    def decode(self, heatmap, rot, dim, center, height, vel, filter=False):
        ...
        baurst_shapes = {
            "pos": torch.cat([center, height], dim=1).clone().permute((0, 2, 1)),
            "dims": dim.clone()[:, 0:3, :].permute((0, 2, 1)),
            "rot": rot.clone().permute((0, 2, 1)),
            "probs": final_scores.clone()[..., None],
        }

        if vel is None:
            final_box_preds = torch.cat(
                [center, height, dim, rot_angle], dim=1
            ).permute(0, 2, 1)
        else:
            final_box_preds = torch.cat(
                [center, height, dim, rot_angle, vel], dim=1
            ).permute(0, 2, 1)
        ...
        # if filter is False:
        return (predictions_dicts, baurst_shapes)

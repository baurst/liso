...
from mmdet3d.core.bbox.samplers.iou_neg_piecewise_sampler import IoUNegPiecewiseSampler
from mmdet3d.core.bbox.transforms import bbox3d2result, bbox3d2roi
from mmdet.core import AssignResult, build_assigner, build_sampler
...
class PointRCNNRoIHead(Base3DRoIHead):
    ...
    def init_assigner_sampler(self):
        ...
        if self.train_cfg:
            if isinstance(self.train_cfg.assigner, (dict, DictConfig)):
                ass_cfg = dict(self.train_cfg.assigner)
                ass_cfg["iou_calculator"] = dict(ass_cfg["iou_calculator"])
                self.bbox_assigner = build_assigner(ass_cfg)
            elif isinstance(self.train_cfg.assigner, list):
                self.bbox_assigner = [
                    build_assigner(res) for res in self.train_cfg.assigner
                ]
            sampler_cfg = dict(self.train_cfg.sampler)
            sampler_cfg["neg_piece_fractions"] = list(
                sampler_cfg["neg_piece_fractions"]
            )
            sampler_cfg["neg_iou_piece_thrs"] = list(sampler_cfg["neg_iou_piece_thrs"])
            self.bbox_sampler = build_sampler(sampler_cfg)

    def forward_train(
        self, feats_dict, input_metas, proposal_list, gt_bboxes_3d, gt_labels_3d
    ):
        ...
        sample_results = self._assign_and_sample(
            proposal_list, gt_bboxes_3d, gt_labels_3d
        )
        ...
        if "loss_bbox" in bbox_results:
            ...
            print("Sth weird happened - No loss_bbox in bbox_results")
    ...
    def _bbox_forward_train(self, features, points, sampling_results):
        ...
        try:
            ...
            print("Something wrong with the forward process of bbox_head")
            print(e)
            bbox_results = {}
        return bbox_results
    ...
    def _bbox_forward(self, features, points, batch_size, rois):
        ...
        """
        pooled_point_feats = self.point_roi_extractor(
            features, points, batch_size, rois
        )
        ...
        for batch_idx in range(len(proposal_list)):
            ...
            if isinstance(self.bbox_assigner, list):
                ...
                assign_result = self.bbox_assigner.assign(
                    cur_boxes.tensor, cur_gt_bboxes.tensor, gt_labels=cur_gt_labels
                )

            # sample boxes
            sampling_result = self.bbox_sampler.sample(
                assign_result, cur_boxes.tensor, cur_gt_bboxes.tensor, cur_gt_labels
            )

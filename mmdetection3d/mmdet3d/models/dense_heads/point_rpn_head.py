...
class PointRPNHead(BaseModule):
    ...
    def class_agnostic_nms(self, obj_scores, sem_scores, bbox, points,
        ...
        general_cfg = self.test_cfg if not self.training else self.train_cfg
        ...
        if general_cfg.score_thr is not None:
            score_thr = self.test_cfg.score_thr
            keep = (obj_scores >= score_thr)
            obj_scores = obj_scores[keep]
            sem_scores = sem_scores[keep]
            # bbox = bbox.tensor[keep] # this looks like a bug? -> set score_thr to None
            bbox = bbox[keep] # this looks like a bug? -> set score_thr to None

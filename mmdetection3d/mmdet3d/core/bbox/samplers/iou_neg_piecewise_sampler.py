            ...
            for piece_inds in range(self.neg_piece_num):
                if piece_inds == self.neg_piece_num - 1:  # for the last piece
                    ...
                    piece_expected_num = min(
                        num_expected,
                        math.ceil(num_expected * self.neg_piece_fractions[piece_inds])
                        + extend_num,
                    )
                ...
                if len(piece_neg_inds) < piece_expected_num:
                    ...
                    if piece_inds == self.neg_piece_num - 1:
                        ...
                        # previous pieces
                            ...
                            if high == 0:
                                print(
                                    "WARNING: this is not supposed to happen, dont understand"
                                )
                                print(
                                    "We are about to torch.randint with low == high == 0 -> would lead to error"
                                )
                                print(
                                    "Replacing neg_inds_choice with all zeros: ",
                                    num_expected,
                                )
                                neg_inds_choice = torch.zeros(
                                    num_expected, dtype=torch.long, device=piece_neg_inds.device
                                )
                            else:
                                rand_idx = torch.randint(
                                    low=0,
